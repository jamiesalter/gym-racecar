...
class Car:
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        for wx, wy in WHEELPOS:
            ...
            w.vr = 0.0 # speed of the wheel assuming no slip
            w.vf = 0.0 # forward speed
            w.vs = 0.0 # side speed
            w.skid_start = None
    ...
    def step(self, dt):
        for w in self.wheels:
            ...
            grass = True
            friction_limit = FRICTION_LIMIT*0.6  # Grass friction if not on a road tile
            for tile in w.tiles:
                # if we're on a road tile, lower the friction (we loop because we could be on multiple tiles)
            ...
            v = w.linearVelocity
            w.vf = forw[0]*v[0] + forw[1]*v[1]  # forward speed (these are dot products)
            w.vs = side[0]*v[0] + side[1]*v[1]  # side speed
            ...
            w.vr = w.omega*w.wheel_rad  # rotating wheel speed
            f_force = -w.vf + w.vr        # force direction is direction of speed difference
            p_force = -w.vs
            ...
            # Simple drag in opposite direction to velocity
            v2 = np.array([0.0, 0.0])
            v2[0] = np.sign(v[0]) * v[0]**2
            v2[1] = np.sign(v[1]) * v[1]**2
            ...
            w.ApplyForceToCenter( (
                p_force*side[0] + f_force*forw[0] + drag[0],
                p_force*side[1] + f_force*forw[1] + drag[1]), True )
