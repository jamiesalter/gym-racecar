...
def main():
    ...
    learning_rate = 1e-4  # The rate at which we learn from our results to compute the new weights. A higher rate
    # means we react more to results and a lower rate means we don’t react as strongly to each result.
    ...
    if resume:
        weights = pickle.load(open('save_increased_learning.p', 'rb'))
    ...
    while True:
        ...
        if done:  # an episode finished
            ...
            if episode_number % batch_size == 0:
                ...
                print("episode %d. updating weights" % (episode_number))

            episode_hidden_layer_values, episode_observations, episode_gradient_log_ps, episode_rewards = [], [], [], []  # reset values
            observation = env.reset()  # reset env
            running_reward = reward_sum if running_reward is None else running_reward * 0.99 + reward_sum * 0.01
            print('resetting env. episode reward total was %d'
                  '. running mean: %f' % (reward_sum, running_reward))
            if episode_number % 100 == 0: pickle.dump(weights, open('save_increased_learning.p', 'wb'))
            reward_sum = 0
            prev_processed_observations = None
